...
#pragma config(Motor,  port6,           UpTower,       tmotorVex393_MC29, openLoop)
#pragma config(Motor,  port7,           DownTower,     tmotorVex393_MC29, openLoop)
...
{
	motor[UpTower]  = power;
	motor[DownTower]  = -power;
	wait1Msec(time);
}
void ArmRise(int power, int time)
{
	motor[Arm] = power;
	wait1Msec(time);
}
void ClawSet(int power, int time)
{
	motor[Claw] = power;
	wait1Msec(time);
}
...
task usercontrol()
{
	...
	while (true)
	{
		...
		{
			motor[UpTower] = motor[DownTower]  =  0;
		}
		if(vexRT[Btn7U] == 1)
		{
			motor[Arm] = 127;
